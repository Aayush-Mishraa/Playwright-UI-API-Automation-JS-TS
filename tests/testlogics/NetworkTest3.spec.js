...
test("Login into new Applcation", async ({ page }) => {
    ...
    //abrot method is used to block the network request for the specified resource type. In this case, it is blocking all the image requests (jpg, jpeg, png) from being loaded on the page. This can help to speed up the test execution and reduce the amount of data being transferred over the network, especially if the page contains a lot of images.
    page.on("response", response => console.log(response.url()));
    page.on("response", response => console.log(response.url(), response.status()));
    //these two event listeners are used to log the URL and status of each network response received by the page. The first listener logs only the URL of the response, while the second listener logs both the URL and the HTTP status code of the response. This can be useful for debugging and analyzing the network traffic during test execution.
    // Listen for all network requests and log their URL's
    ...
    // //this method waits for the network to be idle, which means that there are no more network requests being made. This is useful to ensure that the page has fully loaded before proceeding with the test.
